 ...
 * "mgba_libretro.dll" / "mgba_libretro.so". If enabled, it loads:
 *
 *     <save_path>/<rom-name-without-extension>.<state_ext>
 *
 * mGBA supports GB, GBC, and GBA games, so the user can specify multiple
 * save_path entries (one per system) - the core tries them top-to-bottom.
 *
 * A line is written to "autoload.log" beside the core for diagnostics.
 *
 * Windows : uses GetModuleHandleExA / GetModuleFileNameA (from <windows.h>,
 *           which is already included via mgba-util/common.h).
 * POSIX   : uses dladdr() from <dlfcn.h> (Linux, macOS, Android).
 *
 * NOTE: mGBA's mgba-util/common.h already includes <windows.h> on Windows
 * and defines strcasecmp/_stricmp, so we don't need inline Win32 decls.    */
...
#ifdef _WIN32
#define AUTOLOAD_FROM_ADDRESS        GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS
#define AUTOLOAD_UNCHANGED_REFCOUNT  GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT
#endif
...
/* Unwrap RetroArch's RASTATE container. Returns true if the file had a
 * RASTATE header; *out_data points to the MEM block inside, *out_size is
 * its length. Returns false if the file is not a RASTATE container (caller
 * should use the raw file). */
...
/* ---- Per-ROM cheats (mGBA main RAM via retro_get_memory_data) ---------- *
 ...
 *
 * mGBA supports GB, GBC, and GBA games. The user can specify multiple
 * cheats_path entries (one per system) - the core tries them top-to-bottom.
 *
 ...
 *   - GB/GBC: WRAM at CPU $C000-$DFFF (8 KB, or 32 KB on CGB)
 *   - GBA:    EWRAM at $02000000-$0203FFFF (256 KB)
 *
 * Hotkeys:
...
void retro_run(void) {
	...
	}
	{
		...
		bool num1_is_pressed = inputCallback(0, RETRO_DEVICE_KEYBOARD, 0, RETROK_KP1);
	...
	/* ---- Hotkey: Numpad 2 -> apply oneshot cheats once ---- */
	{
		...
		bool num2_is_pressed = inputCallback(0, RETRO_DEVICE_KEYBOARD, 0, RETROK_KP2);
	...
	/* ---- Hotkey: Numpad 0 -> toggle const cheats on/off ---- */
	{
		...
		bool num0_is_pressed = inputCallback(0, RETRO_DEVICE_KEYBOARD, 0, RETROK_KP0);
	...
	/* ---- Hotkey: Numpad 3 -> load save state WITHOUT reset ---- */
	{
		...
		bool num3_is_pressed = inputCallback(0, RETRO_DEVICE_KEYBOARD, 0, RETROK_KP3);
	...
	/* ---- Hotkey: Numpad 7 -> previous save state slot ---- */
	{
		...
		bool num7_is_pressed = inputCallback(0, RETRO_DEVICE_KEYBOARD, 0, RETROK_KP7);
	...
	/* ---- Hotkey: Numpad 8 -> next save state slot ---- */
	{
		...
		bool num8_is_pressed = inputCallback(0, RETRO_DEVICE_KEYBOARD, 0, RETROK_KP8);
	...
	/* ---- Hotkey: Numpad 9 -> reload config + cheats ---- */
	{
		...
		bool num9_is_pressed = inputCallback(0, RETRO_DEVICE_KEYBOARD, 0, RETROK_KP9);
